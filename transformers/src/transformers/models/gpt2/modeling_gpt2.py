...
@add_start_docstrings(
    "The bare GPT2 Model transformer with sinusoidal position encodings outputting raw hidden-states without any specific head on top.",
    GPT2_START_DOCSTRING,
)
class GPT2SinusoidalPosModel(GPT2Model):
    def __init__(self, config):
        super().__init__(config)
        self.pe = PositionalEncoding(
            self.embed_dim, dropout=config.embd_pdrop, max_len=config.max_position_embeddings, batch_first=True
        )
    ...
    def forward(
        ...
        hidden_states = self.pe(inputs_embeds)
