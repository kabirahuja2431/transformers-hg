                        ...
                        for binary_rule in binary_rules:
                            if binary_rule[1] == nt_left and binary_rule[2] == nt_right:
                                ...
                                for d in range(i, i + j):
                                    if (
                                        nt_left in inside_probs[i][d].keys()
                                        and nt_right
                                        in inside_probs[d + 1][i + j].keys()
                                    ):
                                        sum_prob += (
                                            binary_rule[3]
                                ...
                                if sum_prob > 0:
                                    if binary_rule[0] in inside_probs[i][i + j].keys():
                                        inside_probs[i][i + j][
                                            binary_rule[0]
                                        ] += sum_prob
                                    else:
            ...
            if (i + j) < len(words):
                ...
                for nt_start in nts:
                    for nt_right in nts:
                        for binary_rule in binary_rules:
                            if (
                                binary_rule[0] == nt_start
                                and binary_rule[2] == nt_right
                                and binary_rule[1] != nt_right
                            ):
                                sum_prob = 0
                                # add up probabilities corresponding to the
                                # rule
                                for e in range(i + j + 1, len(words)):
                                    if (
                                        nt_start in outside_probs[i][e].keys()
                                        and nt_right
                                        in inside_probs[i + j + 1][e].keys()
                                    ):
                                        sum_prob += (
                                            binary_rule[3]
                ...
                for nt_start in nts:
                    for nt_left in nts:
                        for binary_rule in binary_rules:
                            if binary_rule[0] == nt_start and binary_rule[1] == nt_left:
                                sum_prob = 0
                                # add up probabilities corresponding to the
                                # rule
                                for e in range(0, i):
                                    if (
                                        nt_start in outside_probs[e][i + j].keys()
                                        and nt_left in inside_probs[e][i - 1].keys()
                                    ):
                                        sum_prob += (
                                            binary_rule[3]
                                ...
                                if sum_prob > 0:
                                    if binary_rule[2] in outside_probs[i][i + j].keys():
                                        outside_probs[i][i + j][
                                            binary_rule[2]
                                        ] += sum_prob
                                    else:
    ...
    for binary_rule in binary_rules:
        ...
        for i in range(0, len(words)):
            for j in range(i + 1, len(words)):
                if binary_rule[0] in outside_probs[i][j].keys():
                    ...
                    for d in range(i, j):
                        if (
                            binary_rule[1] in inside_probs[i][d].keys()
        ...
        for i in range(0, len(words)):
            for j in range(i, len(words)):
                if (
                    binary_rule[0] in outside_probs[i][j].keys()
                    and binary_rule[0] in inside_probs[i][j].keys()
                ):
                    denominator += (
                        outside_probs[i][j][binary_rule[0]]
...
def training(sents, unary_rules, binary_rules, nts, i):
...
def read_grammar():
        ...
        if is_probabilistic:
            ...
            for line in lines:
                ...
                if not (len(l) == 5 or len(l) == 4):
                    print(
                        "'pcfg.txt' should contain probabilities for each \
    ...
    # no grammar supplied
        ...
        for nt_start in nts:
            for nt_left in nts:
                for nt_right in nts:
                    binary_rules.append((nt_start, nt_left, nt_right, prob))
            ...
            if is_probabilistic:
                for pline in plines:
                    if len(pline.split()) != 4:
                        # print(pline)
                        print(
                            "'pos.txt' should contain probabilities for each \
                        rule or no probabilities at all."
                        )
                        sys.exit(-1)
                for pline in plines:
