__author__ = "zhouyang"
from inside_outside.inside_outside_algorithm import Inside_Outside_ALGO
from collections import defaultdict
from inside_outside.inside_outside_algorithm import ishan
            ...
            for i in range(1, self.n + 1):
                for k in range(i, self.n + 1):
                    for j in range(k + 1, self.n + 1):
                        count[
                            tuple([A, B, C])
                        ] += self.inside_outside_instance.get_u_binary(A, B, C, i, k, j)
            count[tuple([A, B, C])] /= self.inside_outside_instance.Z
        for A, w in self.cfg.unary_rules:
            for i in range(1, self.n + 1):
                if w == self.sentence[i - 1]:
                    count[tuple([A, w])] += self.inside_outside_instance.get_u_unary(
                        A, i
                    )
